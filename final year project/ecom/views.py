...
def admin_dashboard_view(request):
    ...
    # Product views
    product_view_count = models.Product.objects.count()  # or use .aggregate() for details

    # Weekly orders
    one_week_ago = timezone.now() - timedelta(days=7)
    weekly_order_count = models.Orders.objects.filter(order_date__gte=one_week_ago).count()
    ...
    context = {
        ...
        'weeklyordercount': weekly_order_count,
...
def admin_add_product_view(request):
    if request.method == 'POST':
        ...
        images_form = forms.ProductImagesForm(request.POST, request.FILES)
        print('*'*20)
        print('Posting data')
        print('*'*20)
        if product_form.is_valid() and images_form.is_valid():
            ...
            if 'additional_images' in request.FILES:
                ...
                print('*'*20)
                print('No additional images provided')
                print('*'*20)
        ...
        images_form = forms.ProductImagesForm()
...
def update_product_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if productForm.is_valid():
            ...
            models.Product_Images.objects.filter(product=product).delete()

            # Save new additional images
            if 'additional_images' in request.FILES:
                for img in request.FILES.getlist('additional_images'):
                    models.Product_Images.objects.create(product=product, image=img)
    ...
    return render(request, 'ecom/admin_update_product.html', {
        ...
        'existing_images': models.Product_Images.objects.filter(product=product)
    })
...
def customer_address_view(request):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        product_count_in_cart=0
    customer = models.Customer.objects.get(user=request.user)  # Assuming Customer is linked to User
    existing_orders = models.Orders.objects.filter(customer=customer).exists()
    if False:
    # if existing_orders:
        ...
        if request.method == 'POST':
            ...
            if addressForm.is_valid():
                ...
                return response
        return render(request, 'ecom/address_pick.html',{'addressForm':addressForm,'product_in_cart':product_in_cart,'product_count_in_cart':product_count_in_cart})
    else:
        addressForm = forms.AddressForm()
        if request.method == 'POST':
            addressForm = forms.AddressForm(request.POST)
...
def edit_profile_view(request):
    customer=models.Customer.objects.get(user_id=request.user.id)
    user=models.User.objects.get(id=customer.user_id)
    userForm=forms.CustomerUpdateForm(instance=user)
    customerForm=forms.CustomerForm(instance=customer)
    mydict={'userForm':userForm,'customerForm':customerForm}
    if request.method=='POST':
        userForm=forms.CustomerUpdateForm(request.POST,instance=user)
...
# @login_required(login_url='customerlogin')
def product_description_view(request, pk):
    product = models.Product.objects.get(pk=pk)
    images = models.Product_Images.objects.filter(product=product)
    if request.method == "POST":
        ...
        if comment_text:
            ...
            return redirect('product-description', pk=pk)  # Redirect to same page after submitting

    # To display comments for this product (if linking later with product)
    # To display comments for this product (if linking later with product)
    comments = models.Comment.objects.filter(product_id=pk).order_by('-date')
    ...
    return render(request, 'ecom/product_description.html', context)